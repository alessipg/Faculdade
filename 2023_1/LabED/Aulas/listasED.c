...
void travessiaRecursiva(TNo *prim);
...
int main()
{
   ...
   * a) por valor: em geral, quando não precisamos alterar o  
   *    encadeamento da lista, passamos o ponteiro para o   
   *    primeiro por valor;
...
TNo* pesquisaPrim(TNo *prim, int k)
{
  ...
  if (prim->chave == k)
   {
     printf("O k informado eh igual ao campo chave do 1o\n");
     return prim;
  }
 else return NULL;
}
...
void checaLista(TNo *prim)
{
  ...
  {
    printf("A lista dada eh vazia!\n");
    return;
  }
//  else
  if (prim->prox == NULL)
    printf("A lista eh unitaria!\n");
  else
    printf("A lista tem mais de um no!\n");
}
...
void printPrim(TNo *primeiro)
{
 if (primeiro == NULL)
  ...
  printf("O valor do campo chave do 1o no eh: %d\n", primeiro->chave);
}
